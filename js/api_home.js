...
const API = (() => {
  ...
  function saveSession(data) {
    const s = { token: data.session_id, account_id: data.account_id, account_type: data.account_type, display_label: data.display_label, tier_id: data.tier_id, tier_name: data.tier_name, store_id: data.store_id, dept_id: data.dept_id, user_id: data.user_id || '', display_name: data.display_name || '', full_name: data.full_name || '', expires_at: data.expires_at, position_id: data.position_id || '', position_name: data.position_name || '', position_level: data.position_level || 99, store_assignments: data.store_assignments || [] };
  ...
  return {
    ...
    selectStore: (temp_token, store_id) => post('select_store', { temp_token, store_id }),
    ...
    // Layered Permission (v2.0)
    adminGetPositions: () => post('admin_get_positions', tb()),
    adminGetBasePermissions: () => post('admin_get_base_permissions', tb()),
    adminUpdateBasePermission: (position_id, module_id, access_level) => post('admin_update_base_permission', tb({ position_id, module_id, access_level })),
    adminGetDeptOverrides: (f = {}) => post('admin_get_dept_overrides', tb(f)),
    adminUpdateDeptOverride: (data) => post('admin_update_dept_override', tb(data)),
    adminGetStoreAssignments: (f = {}) => post('admin_get_store_assignments', tb(f)),
    adminSetStoreAssignment: (data) => post('admin_set_store_assignment', tb(data)),
    adminRemoveStoreAssignment: (data) => post('admin_remove_store_assignment', tb(data)),
  };
})();
